...
function Initiate(){
  ...
  if (actualCode.localeCompare(wirteCode) != 0) {
    ...
    db.collection('New-Flash-Base').doc(subjectid).set({
    SYC_list:SYC_list,
    })
    .then(function() {
...
function saveFeedback1(){
  ...
  if (subjectid.charAt(subjectid.length-1)<TotalParts){
    db.collection('New-Flash-Base').doc(subjectid).update({
      feedback:document.getElementById("BaseQ").value,
    })
...
function playSYC() {
  ...
  if (SYCtrial>0){  playWhitenoise (SYCSOA[SYC_list[SYCtrial-1]%SYCSOA.length] *ExpStimuli/1000-0.1+1.5);
...
function animateSYC() {
   canvas.style.display='none';
   if ([SYC_list[SYCtrial-1]]==0) {changediv(SYCintro_prac,SYCISI01);
      setTimeout(function(){ changediv(SYCISI01,Question);}, SYCSOA[SYC_list[SYCtrial-1]%SYCSOA.length] *ExpStimuli-20)
    } else if ([SYC_list[SYCtrial-1]]==1){changediv(SYCintro,SYCISI11);
      setTimeout(function(){ changediv(SYCISI11,Question);}, SYCSOA[SYC_list[SYCtrial-1]%SYCSOA.length] *ExpStimuli-20)
    } else if ([SYC_list[SYCtrial-1]]==2){changediv(SYCintro,SYCISI21);
      setTimeout(function(){ changediv(SYCISI21,Question);}, SYCSOA[SYC_list[SYCtrial-1]%SYCSOA.length] *ExpStimuli-20)
    } else if ([SYC_list[SYCtrial-1]]==3) {changediv(SYCintro_prac,SYCISI02);
       setTimeout(function(){ changediv(SYCISI02,Question);}, SYCSOA[SYC_list[SYCtrial-1]%SYCSOA.length] *ExpStimuli-20)
     } else if ([SYC_list[SYCtrial-1]]==4){changediv(SYCintro,SYCISI12);
       setTimeout(function(){ changediv(SYCISI12,Question);}, SYCSOA[SYC_list[SYCtrial-1]%SYCSOA.length] *ExpStimuli-20)
     } else if ([SYC_list[SYCtrial-1]]==5){changediv(SYCintro,SYCISI22);
       setTimeout(function(){ changediv(SYCISI22, Question);}, SYCSOA[SYC_list[SYCtrial-1]%SYCSOA.length] *ExpStimuli-20)
     }
}
...
function datareportSYC() {
  ...
  document.removeEventListener('keydown',datarecordedSYC,true);
  if (SYC_list[SYCtrial-1]<=3 && document.getElementById("auditest").value==1+SYC_list[SYCtrial-1]){
    document.getElementById('Correct').style.display='block';
    document.getElementById('Wrong').style.display='none';
  } else if (SYC_list[SYCtrial-1]>3 && document.getElementById("auditest").value==0){
    document.getElementById('Correct').style.display='block';
    document.getElementById('Wrong').style.display='none';
  }
  else{
  ...
  for (i = 1; i <= ExpStimuli; i++) {
    Stimuli_SYC.push(i*SYCSOA[SYC_list[SYCtrial-1]%SYCSOA.length]);
  }
  // report and rest
  if (typeof RTtime !== 'undefined' && RTtime.length > 0) {
    // find the valid trial (RT<1)
    ...
    if (Math.abs(closest-Stimuli_SYC[i])<450){ Response_RT.push(closest-Stimuli_SYC[i]);}
    }
